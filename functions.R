...
sig_heatmap <- function(all_patients_result, title,clustering_distance =  "euclidean", annotation = NULL, silent = F, sign_matrix = NULL,color=NULL,...) {
  my_fun <- function(p) {                     
    asterisks_vec = p
  ...
  if(!is.null(sign_matrix)){
    sign_matrix[,colnames(all_patients_result)]
